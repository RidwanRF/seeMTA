	...
	function ()
		for TextureName, ShouldBeLoaded in pairs(TexturesToBeLoaded) do
			if ShouldBeLoaded then
				local FilePath = UnlockFile("Files/Images/" .. TextureName .. ".Strong", FileKeys[TextureName])

				if fileExists("Files/Images/" .. TextureName .. ".Strong") then
...
function TryToStartSantaOpening(DbID)
	...
	if DbID then
		triggerServerEvent("äłÄÍÄ<đ\|Ä\|ÄäđĐ$äđĐ", localPlayer, localPlayer, "dbID", DbID, 1, "octansSeeMTA")
	end
...
function StartSantaOpening()
	...
	exports.see_chat:localActionC(localPlayer, "kinyit egy ajándékot.")
end
...
function RenderMinigame()
	...
	for I = CarouSelFirstVisibleItem, CarouselItemsNum + CarouSelFirstVisibleItem do
		...
		if ItemId then
			...
			dxDrawImage(ItemPosnX, ItemPosnY, ItemSizeX, ItemSizeY, ":see_items/files/items/" .. ItemId - 1 .. ".png", 0, 0, 0, tocolor(255, 255, 255, DerivedAlpha * AlphaFactor))
	...
	if CarouselTextOpacity > 0 then
		...
		if ItemId then
			...
			-- Item name shadow
			dxDrawText(
				exports.see_items:getItemName(ItemId),
				ItemsLinePosnX + 1,
	...
	if CarouSelFinishTimer then
		...
		elseif ElapsedTime > 0 then
			...
			if GameState ~= "GiveReward" then
				...
				if ItemId then
					triggerServerEvent("$$$$$$$$$$$$$$$", localPlayer, localPlayer, ItemId, 1, false, false, false, false, "faszhuszar.net")

					if GoldItems[ItemId] then
						outputChatBox("#d75959[StrongMTA - Mikulás]:#ffffff Nyereményed: #e8c55a" .. exports.see_items:getItemName(ItemId), 255, 255, 255, true)
					else
						outputChatBox("#d75959[StrongMTA - Mikulás]:#ffffff Nyereményed: #7cc576" .. exports.see_items:getItemName(ItemId), 255, 255, 255, true)
					end
				end
