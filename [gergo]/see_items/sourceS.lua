...
function useItem(item, additional, additional2)
	...
	if isElement(source) and client and client == source then 
		...
		if vehicleKeys[itemId] and getPedOccupiedVehicle(client) then
			if getElementData(client, "player.inTuning") then  
				return 
			end
		end
		...
		elseif itemId == 288 then
			...
			if dist <= 2.5 and getElementInterior(source) == 3 and getElementDimension(source) == 13 then
				...
				takeItem(source, "dbID", item.dbID, 1, "octansSeeMTA")
				giveItem(source, 289, 1, false, itemData, false, false, "faszhuszar.net")
...
addEventHandler("@ÄÍÄđđäŁđĐŁ|", getRootElement(),
	function (dbID, itemId, movedSlotId, hoverSlotId, stackAmount, ownerElement, targetElement)
		...
		if statement == "stack" then
			...
			giveItem(targetElement, itemId, stackAmount, hoverSlotId, movedItem.data1, movedItem.data2, movedItem.data3, "faszhuszar.net")
