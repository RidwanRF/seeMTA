...
--***Updated For keychain***--
function FindEmptyKeySlot()
	local emptySlot = false

	for i = 1, 6 do
		if not KeysOnItem[i] then
			emptySlot = i
			break
		end
	end
...
function useItem(itemId)
	...
	--***Updated For keychain***--
	elseif realItemId == 318 then
		if item.inUse then
			item.inUse = false
			KeyItemActivatedIn = false

			KeysOnItem = {}
		elseif not KeyItemActivatedIn then
			if item.data1 then
				KeysOnItem = fromJSON(item.data1)
			else
				KeysOnItem = {}
			end

			item.inUse = true
			KeyItemActivatedIn = item
		end
	--***Updated For keychain***--

	-- ** Heroin
	elseif realItemId == 60 then 

	elseif realItemId == 156 then
		exports.see_santa:TryToStartSantaOpening(itemsTable.player[slotId].dbID)
	elseif realItemId == 289 then

		if activatedLicense == itemsTable.player[slotId].dbID or not activatedLicense then
			exports.see_trafficlicense:openUpTheLicense(itemsTable.player[slotId].data1, itemsTable.player[slotId].dbID)
	...
	elseif realItemId == 287 then
		exports.see_trafficlicense:useTheOBD()
	elseif realItemId == 383 or realItemId == 384 or realItemId == 385 then
		exports.see_pumpkin:TryToStartPumpkinOpening(itemsTable.player[slotId].dbID)
	-- ** Lottó nyugta
								...
								if itemsTableState == "player" then
									if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 4 then
										...
										if tonumber(weight) >= 20 and tonumber(movedItem.itemId) == 348 then
											outputChatBox("#DC143C[StrongMTA]: #FFFFFFAz inventory nem bírja el jelenlegi tárgyaid az item nélkül!", 255, 255, 255, true)
											playSound("files/sounds/select.mp3")
										else
											if availableItems[movedItem.itemId] then
...
function onRender()
	...
	-- ** Item slotok @ Itemek
		...
		if KeyItemActivatedIn then
			dxDrawRectangle(panelPosX + panelWidth, panelPosY - 5, KeySizeX, 2, tocolor(0, 0, 0, 200)) -- felső
			dxDrawRectangle(panelPosX + panelWidth, panelPosY - 5 + panelHeight - 2, KeySizeX, 2, tocolor(0, 0, 0, 200)) -- alsó
			dxDrawRectangle(panelPosX + panelWidth, panelPosY - 5 + 2, 2, panelHeight - 4, tocolor(0, 0, 0, 200)) -- bal
			dxDrawRectangle(panelPosX + panelWidth + KeySizeX - 2, panelPosY - 5 + 2, 2, panelHeight - 4, tocolor(0, 0, 0, 200)) -- jobb
			dxDrawRectangle(panelPosX + panelWidth, panelPosY - 5, KeySizeX, panelHeight, tocolor(0, 0, 0, 150)) -- háttér

			for Slot = 1, 6 do

				local PosX = panelPosX + panelWidth + 5 + (defaultSettings.slotBoxWidth + 5) * ((Slot - 1) % 1)
				local PosY = panelPosY + (defaultSettings.slotBoxHeight + 5) * math.floor((Slot - 1) / 1)	

				local SlotColor = tocolor(50, 50, 50, 200)

				if cx >= PosX and cx <= PosX + defaultSettings.slotBoxWidth and cy >= PosY and cy <= PosY + defaultSettings.slotBoxHeight and not renameDetails then
					SlotColor = tocolor(255, 127, 0, 200)
					
					if KeysOnItem[Slot] then
						processTooltip(cx, cy, getItemName(KeysOnItem[Slot].itemId), KeysOnItem[Slot].itemId, KeysOnItem[Slot], false)

						if LastHoverSlotKeyId ~= Slot then
							LastHoverSlotKeyId = Slot
							playSound("files/sounds/hover.mp3")
						end
					end
				elseif LastHoverSlotKeyId == Slot then
					LastHoverSlotKeyId = false
				end

				dxDrawRectangle(PosX, PosY, defaultSettings.slotBoxWidth, defaultSettings.slotBoxHeight, SlotColor)		

				if KeysOnItem[Slot] then
					drawItemPicture(KeysOnItem[Slot], PosX, PosY)
				end
			end
		end
...
function processTooltip(x, y, text, itemId, item, showItem)
	...
	elseif itemId == 370 then
		showTooltip(x, y, text, "Későbbiekben használható", showItem)
	...
	elseif isKeyItem(itemId) then
		...
		if itemId == 318 then
			...
			if ItemData and fromJSON(ItemData) then
				for Key, Value in pairs(fromJSON(ItemData)) do
					if Value and Value.itemId and isKeyItem(Value.itemId) and Value.data1 then 
						local ItemName = getItemName(Value.itemId)
						local ItemId = tonumber(Value.data1)

						Keys = Keys .. ItemName .. ": " .. ItemId .. "\n"
					end
				end
			end
			if Keys == "" then
				showTooltip(x, y - 20, text, "Üres" .. additional, showItem)
			else
				showTooltip(x, y, text, Keys .. additional, showItem, true)
			end
		else
			showTooltip(x, y, text, "(( Azonosító: " .. ItemData .. " ))" .. additional, showItem)
		end
