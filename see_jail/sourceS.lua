...
addEventHandler("updateJailTime", getRootElement(),
	function ()
		if isElement(source) then 
			local adminJailTime = tonumber(getElementData(source, "acc.adminJailTime"))

			--print("triggered")
			dbExec(connection, "UPDATE accounts SET adminJailTime = ? WHERE accountId = ?", adminJailTime, getElementData(source, "char.accID"))
		end		
	end 
)
